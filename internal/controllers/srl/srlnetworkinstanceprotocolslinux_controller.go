...
const (
	// Errors
	errUnexpectedNetworkinstanceProtocolsLinux       = "the managed resource is not a NetworkinstanceProtocolsLinux resource"
	errKubeUpdateFailedNetworkinstanceProtocolsLinux = "cannot update NetworkinstanceProtocolsLinux"
	errReadNetworkinstanceProtocolsLinux             = "cannot read NetworkinstanceProtocolsLinux"
	errCreateNetworkinstanceProtocolsLinux           = "cannot create NetworkinstanceProtocolsLinux"
	errUpdateNetworkinstanceProtocolsLinux           = "cannot update NetworkinstanceProtocolsLinux"
	errDeleteNetworkinstanceProtocolsLinux           = "cannot delete NetworkinstanceProtocolsLinux"
)

// SetupNetworkinstanceProtocolsLinux adds a controller that reconciles NetworkinstanceProtocolsLinuxs.
func SetupNetworkinstanceProtocolsLinux(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupNetworkinstanceProtocolsLinux(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.NetworkinstanceProtocolsLinuxGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangNetworkinstanceProtocolsLinux(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.NetworkinstanceProtocolsLinuxGroupVersionKind),
		managed.WithPollInterval(nddcopts.Poll),
		managed.WithExternalConnecter(&connectorNetworkinstanceProtocolsLinux{
			log:          nddcopts.Logger,
			kube:         mgr.GetClient(),
		...
		managed.WithValidator(&validatorNetworkinstanceProtocolsLinux{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("NetworkinstanceProtocolsLinux-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	NetworkinstanceProtocolsLinuxHandler := &EnqueueRequestForAllNetworkinstanceProtocolsLinux{
	...
	return srlv1alpha1.NetworkinstanceProtocolsLinuxGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlNetworkinstanceProtocolsLinux{}).
		Owns(&srlv1alpha1.SrlNetworkinstanceProtocolsLinux{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlNetworkinstanceProtocolsLinux{}}, NetworkinstanceProtocolsLinuxHandler).
		Watches(&source.Channel{Source: events}, NetworkinstanceProtocolsLinuxHandler).
		Complete(r)
}
...
func initYangNetworkinstanceProtocolsLinux(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &NetworkinstanceProtocolsLinux{&yresource.Resource{
...
func (r *NetworkinstanceProtocolsLinux) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsLinux)
	if !ok {
...
func (r *NetworkinstanceProtocolsLinux) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorNetworkinstanceProtocolsLinux struct {
	log          logging.Logger
	deviceSchema *yentry.Entry
	y            yresource.Handler
}

func (v *validatorNetworkinstanceProtocolsLinux) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsLinux)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedNetworkinstanceProtocolsLinux)
	}
	d, err := json.Marshal(&cr.Spec.NetworkinstanceProtocolsLinux)
	if err != nil {
...
func (v *validatorNetworkinstanceProtocolsLinux) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
...
func (v *validatorNetworkinstanceProtocolsLinux) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	//log := v.log.WithValues("resource", mg.GetName())
...
// is called.
type connectorNetworkinstanceProtocolsLinux struct {
	log          logging.Logger
	kube         client.Client
...
func (c *connectorNetworkinstanceProtocolsLinux) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	log := c.log.WithValues("resource", mg.GetName())
	//log.Debug("Connect")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsLinux)
	if !ok {
		return nil, errors.New(errUnexpectedNetworkinstanceProtocolsLinux)
	}
	...
	return &externalNetworkinstanceProtocolsLinux{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalNetworkinstanceProtocolsLinux struct {
...
func (e *externalNetworkinstanceProtocolsLinux) Close() {
	e.client.Close()
}

func (e *externalNetworkinstanceProtocolsLinux) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Observing ...")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsLinux)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errUnexpectedNetworkinstanceProtocolsLinux)
	}
...
func (e *externalNetworkinstanceProtocolsLinux) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsLinux)
	if !ok {
		return errors.New(errUnexpectedNetworkinstanceProtocolsLinux)
	}
...
func (e *externalNetworkinstanceProtocolsLinux) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalNetworkinstanceProtocolsLinux) Delete(ctx context.Context, mg resource.Managed) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Deleting ...")
...
func (e *externalNetworkinstanceProtocolsLinux) GetTarget() []string {
	return e.targets
}

func (e *externalNetworkinstanceProtocolsLinux) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalNetworkinstanceProtocolsLinux) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
