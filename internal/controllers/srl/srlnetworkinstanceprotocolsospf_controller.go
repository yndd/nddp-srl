...
const (
	...
	errUpdateNetworkinstanceProtocolsOspf           = "cannot update NetworkinstanceProtocolsOspf"
...
func SetupNetworkinstanceProtocolsOspf(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupNetworkinstanceProtocolsOspf(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.NetworkinstanceProtocolsOspfGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangNetworkinstanceProtocolsOspf(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.NetworkinstanceProtocolsOspfGroupVersionKind),
		...
		managed.WithValidator(&validatorNetworkinstanceProtocolsOspf{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("NetworkinstanceProtocolsOspf-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	NetworkinstanceProtocolsOspfHandler := &EnqueueRequestForAllNetworkinstanceProtocolsOspf{
	...
	return srlv1alpha1.NetworkinstanceProtocolsOspfGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlNetworkinstanceProtocolsOspf{}).
		Owns(&srlv1alpha1.SrlNetworkinstanceProtocolsOspf{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlNetworkinstanceProtocolsOspf{}}, NetworkinstanceProtocolsOspfHandler).
		Watches(&source.Channel{Source: events}, NetworkinstanceProtocolsOspfHandler).
		Complete(r)
}
...
func initYangNetworkinstanceProtocolsOspf(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &NetworkinstanceProtocolsOspf{&yresource.Resource{
...
func (r *NetworkinstanceProtocolsOspf) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsOspf)
	if !ok {
...
func (r *NetworkinstanceProtocolsOspf) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
func (v *validatorNetworkinstanceProtocolsOspf) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceProtocolsOspf)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedNetworkinstanceProtocolsOspf)
	}
	d, err := json.Marshal(&cr.Spec.NetworkinstanceProtocolsOspf)
	if err != nil {
...
func (c *connectorNetworkinstanceProtocolsOspf) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalNetworkinstanceProtocolsOspf{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}
...
func (e *externalNetworkinstanceProtocolsOspf) Close() {
	e.client.Close()
}
...
func (e *externalNetworkinstanceProtocolsOspf) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	log := e.log.WithValues("Resource", mg.GetName())
	log.Debug("Updating ...")
...
func (e *externalNetworkinstanceProtocolsOspf) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalNetworkinstanceProtocolsOspf) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
