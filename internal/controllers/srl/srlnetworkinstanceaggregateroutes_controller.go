...
const (
	// Errors
	errUnexpectedNetworkinstanceAggregateroutes       = "the managed resource is not a NetworkinstanceAggregateroutes resource"
	errKubeUpdateFailedNetworkinstanceAggregateroutes = "cannot update NetworkinstanceAggregateroutes"
	errReadNetworkinstanceAggregateroutes             = "cannot read NetworkinstanceAggregateroutes"
	errCreateNetworkinstanceAggregateroutes           = "cannot create NetworkinstanceAggregateroutes"
	errUpdateNetworkinstanceAggregateroutes           = "cannot update NetworkinstanceAggregateroutes"
...
func SetupNetworkinstanceAggregateroutes(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupNetworkinstanceAggregateroutes(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.NetworkinstanceAggregateroutesGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangNetworkinstanceAggregateroutes(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.NetworkinstanceAggregateroutesGroupVersionKind),
		managed.WithPollInterval(nddcopts.Poll),
		managed.WithExternalConnecter(&connectorNetworkinstanceAggregateroutes{
			log:          nddcopts.Logger,
			kube:         mgr.GetClient(),
		...
		managed.WithValidator(&validatorNetworkinstanceAggregateroutes{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("NetworkinstanceAggregateroutes-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	NetworkinstanceAggregateroutesHandler := &EnqueueRequestForAllNetworkinstanceAggregateroutes{
	...
	return srlv1alpha1.NetworkinstanceAggregateroutesGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlNetworkinstanceAggregateroutes{}).
		Owns(&srlv1alpha1.SrlNetworkinstanceAggregateroutes{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlNetworkinstanceAggregateroutes{}}, NetworkinstanceAggregateroutesHandler).
		Watches(&source.Channel{Source: events}, NetworkinstanceAggregateroutesHandler).
		Complete(r)
}
...
func initYangNetworkinstanceAggregateroutes(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &NetworkinstanceAggregateroutes{&yresource.Resource{
...
func (r *NetworkinstanceAggregateroutes) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceAggregateroutes)
	if !ok {
...
func (r *NetworkinstanceAggregateroutes) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorNetworkinstanceAggregateroutes struct {
	log          logging.Logger
	deviceSchema *yentry.Entry
	y            yresource.Handler
}

func (v *validatorNetworkinstanceAggregateroutes) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceAggregateroutes)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedNetworkinstanceAggregateroutes)
	}
	d, err := json.Marshal(&cr.Spec.NetworkinstanceAggregateroutes)
	if err != nil {
...
func (v *validatorNetworkinstanceAggregateroutes) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
...
func (v *validatorNetworkinstanceAggregateroutes) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	//log := v.log.WithValues("resource", mg.GetName())
...
// is called.
type connectorNetworkinstanceAggregateroutes struct {
	log          logging.Logger
	kube         client.Client
...
func (c *connectorNetworkinstanceAggregateroutes) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	log := c.log.WithValues("resource", mg.GetName())
	//log.Debug("Connect")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceAggregateroutes)
	if !ok {
		return nil, errors.New(errUnexpectedNetworkinstanceAggregateroutes)
	}
	...
	return &externalNetworkinstanceAggregateroutes{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalNetworkinstanceAggregateroutes struct {
...
func (e *externalNetworkinstanceAggregateroutes) Close() {
	e.client.Close()
}

func (e *externalNetworkinstanceAggregateroutes) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Observing ...")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceAggregateroutes)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errUnexpectedNetworkinstanceAggregateroutes)
	}
...
func (e *externalNetworkinstanceAggregateroutes) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceAggregateroutes)
	if !ok {
		return errors.New(errUnexpectedNetworkinstanceAggregateroutes)
	}
...
func (e *externalNetworkinstanceAggregateroutes) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalNetworkinstanceAggregateroutes) Delete(ctx context.Context, mg resource.Managed) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Deleting ...")
...
func (e *externalNetworkinstanceAggregateroutes) GetTarget() []string {
	return e.targets
}

func (e *externalNetworkinstanceAggregateroutes) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalNetworkinstanceAggregateroutes) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
