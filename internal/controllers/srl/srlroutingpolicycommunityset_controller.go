...
const (
	// Errors
	errUnexpectedRoutingpolicyCommunityset       = "the managed resource is not a RoutingpolicyCommunityset resource"
	errKubeUpdateFailedRoutingpolicyCommunityset = "cannot update RoutingpolicyCommunityset"
	errReadRoutingpolicyCommunityset             = "cannot read RoutingpolicyCommunityset"
	errCreateRoutingpolicyCommunityset           = "cannot create RoutingpolicyCommunityset"
	errUpdateRoutingpolicyCommunityset           = "cannot update RoutingpolicyCommunityset"
...
func SetupRoutingpolicyCommunityset(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupRoutingpolicyCommunityset(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.RoutingpolicyCommunitysetGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangRoutingpolicyCommunityset(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.RoutingpolicyCommunitysetGroupVersionKind),
		managed.WithPollInterval(nddcopts.Poll),
		managed.WithExternalConnecter(&connectorRoutingpolicyCommunityset{
			log:          nddcopts.Logger,
			kube:         mgr.GetClient(),
		...
		managed.WithValidator(&validatorRoutingpolicyCommunityset{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("RoutingpolicyCommunityset-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	RoutingpolicyCommunitysetHandler := &EnqueueRequestForAllRoutingpolicyCommunityset{
	...
	return srlv1alpha1.RoutingpolicyCommunitysetGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlRoutingpolicyCommunityset{}).
		Owns(&srlv1alpha1.SrlRoutingpolicyCommunityset{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlRoutingpolicyCommunityset{}}, RoutingpolicyCommunitysetHandler).
		Watches(&source.Channel{Source: events}, RoutingpolicyCommunitysetHandler).
		Complete(r)
}
...
func initYangRoutingpolicyCommunityset(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &RoutingpolicyCommunityset{&yresource.Resource{
...
func (r *RoutingpolicyCommunityset) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyCommunityset)
	if !ok {
...
func (r *RoutingpolicyCommunityset) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorRoutingpolicyCommunityset struct {
	log          logging.Logger
	deviceSchema *yentry.Entry
	y            yresource.Handler
}

func (v *validatorRoutingpolicyCommunityset) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyCommunityset)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedRoutingpolicyCommunityset)
	}
	d, err := json.Marshal(&cr.Spec.RoutingpolicyCommunityset)
	if err != nil {
...
func (v *validatorRoutingpolicyCommunityset) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
...
func (v *validatorRoutingpolicyCommunityset) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	//log := v.log.WithValues("resource", mg.GetName())
...
// is called.
type connectorRoutingpolicyCommunityset struct {
	log          logging.Logger
	kube         client.Client
...
func (c *connectorRoutingpolicyCommunityset) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	log := c.log.WithValues("resource", mg.GetName())
	//log.Debug("Connect")

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyCommunityset)
	if !ok {
		return nil, errors.New(errUnexpectedRoutingpolicyCommunityset)
	}
	...
	return &externalRoutingpolicyCommunityset{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalRoutingpolicyCommunityset struct {
...
func (e *externalRoutingpolicyCommunityset) Close() {
	e.client.Close()
}

func (e *externalRoutingpolicyCommunityset) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Observing ...")

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyCommunityset)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errUnexpectedRoutingpolicyCommunityset)
	}
...
func (e *externalRoutingpolicyCommunityset) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyCommunityset)
	if !ok {
		return errors.New(errUnexpectedRoutingpolicyCommunityset)
	}
...
func (e *externalRoutingpolicyCommunityset) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalRoutingpolicyCommunityset) Delete(ctx context.Context, mg resource.Managed) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Deleting ...")
...
func (e *externalRoutingpolicyCommunityset) GetTarget() []string {
	return e.targets
}

func (e *externalRoutingpolicyCommunityset) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalRoutingpolicyCommunityset) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
