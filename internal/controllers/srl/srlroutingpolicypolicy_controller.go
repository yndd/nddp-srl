...
const (
	// Errors
	errUnexpectedRoutingpolicyPolicy       = "the managed resource is not a RoutingpolicyPolicy resource"
	errKubeUpdateFailedRoutingpolicyPolicy = "cannot update RoutingpolicyPolicy"
	errReadRoutingpolicyPolicy             = "cannot read RoutingpolicyPolicy"
	errCreateRoutingpolicyPolicy           = "cannot create RoutingpolicyPolicy"
	errUpdateRoutingpolicyPolicy           = "cannot update RoutingpolicyPolicy"
...
func SetupRoutingpolicyPolicy(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupRoutingpolicyPolicy(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.RoutingpolicyPolicyGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangRoutingpolicyPolicy(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.RoutingpolicyPolicyGroupVersionKind),
		managed.WithPollInterval(nddcopts.Poll),
		managed.WithExternalConnecter(&connectorRoutingpolicyPolicy{
			log:          nddcopts.Logger,
			kube:         mgr.GetClient(),
		...
		managed.WithValidator(&validatorRoutingpolicyPolicy{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("RoutingpolicyPolicy-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	RoutingpolicyPolicyHandler := &EnqueueRequestForAllRoutingpolicyPolicy{
	...
	return srlv1alpha1.RoutingpolicyPolicyGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlRoutingpolicyPolicy{}).
		Owns(&srlv1alpha1.SrlRoutingpolicyPolicy{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlRoutingpolicyPolicy{}}, RoutingpolicyPolicyHandler).
		Watches(&source.Channel{Source: events}, RoutingpolicyPolicyHandler).
		Complete(r)
}
...
func initYangRoutingpolicyPolicy(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &RoutingpolicyPolicy{&yresource.Resource{
...
func (r *RoutingpolicyPolicy) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyPolicy)
	if !ok {
...
func (r *RoutingpolicyPolicy) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorRoutingpolicyPolicy struct {
	log          logging.Logger
	deviceSchema *yentry.Entry
	y            yresource.Handler
}

func (v *validatorRoutingpolicyPolicy) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyPolicy)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedRoutingpolicyPolicy)
	}
	d, err := json.Marshal(&cr.Spec.RoutingpolicyPolicy)
	if err != nil {
...
func (v *validatorRoutingpolicyPolicy) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
...
func (v *validatorRoutingpolicyPolicy) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	//log := v.log.WithValues("resource", mg.GetName())
...
// is called.
type connectorRoutingpolicyPolicy struct {
	log          logging.Logger
	kube         client.Client
...
func (c *connectorRoutingpolicyPolicy) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	log := c.log.WithValues("resource", mg.GetName())
	//log.Debug("Connect")

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyPolicy)
	if !ok {
		return nil, errors.New(errUnexpectedRoutingpolicyPolicy)
	}
	...
	return &externalRoutingpolicyPolicy{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalRoutingpolicyPolicy struct {
...
func (e *externalRoutingpolicyPolicy) Close() {
	e.client.Close()
}

func (e *externalRoutingpolicyPolicy) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Observing ...")

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyPolicy)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errUnexpectedRoutingpolicyPolicy)
	}
...
func (e *externalRoutingpolicyPolicy) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlRoutingpolicyPolicy)
	if !ok {
		return errors.New(errUnexpectedRoutingpolicyPolicy)
	}
...
func (e *externalRoutingpolicyPolicy) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalRoutingpolicyPolicy) Delete(ctx context.Context, mg resource.Managed) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Deleting ...")
...
func (e *externalRoutingpolicyPolicy) GetTarget() []string {
	return e.targets
}

func (e *externalRoutingpolicyPolicy) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalRoutingpolicyPolicy) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
