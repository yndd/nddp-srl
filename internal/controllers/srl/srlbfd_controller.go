...
func SetupBfd(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	...
	BfdHandler := &EnqueueRequestForAllBfd{
	...
	return srlv1alpha1.BfdGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlBfd{}}, BfdHandler).
		Watches(&source.Channel{Source: events}, BfdHandler).
		Complete(r)
}
...
func initYangBfd(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
...
func (v *validatorBfd) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlBfd)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedBfd)
	}
...
func (e *externalBfd) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlBfd)
	if !ok {
		return errors.New(errUnexpectedBfd)
	}
...
func (e *externalBfd) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
