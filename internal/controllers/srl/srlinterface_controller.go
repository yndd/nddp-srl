...
const (
	...
	errUpdateInterface           = "cannot update Interface"
...
func SetupInterface(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupInterface(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.InterfaceGroupKind)
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.InterfaceGroupVersionKind),
		...
		managed.WithValidator(&validatorInterface{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("Interface-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	InterfaceHandler := &EnqueueRequestForAllInterface{
	...
	return srlv1alpha1.InterfaceGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlInterface{}}, InterfaceHandler).
		Watches(&source.Channel{Source: events}, InterfaceHandler).
		Complete(r)
}
...
func initYangInterface(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &Interface{&yresource.Resource{
...
func (r *Interface) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlInterface)
	if !ok {
...
func (r *Interface) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
func (v *validatorInterface) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlInterface)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedInterface)
	}
	d, err := json.Marshal(&cr.Spec.Interface)
	if err != nil {
...
func (c *connectorInterface) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalInterface{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}
...
func (e *externalInterface) Close() {
	e.client.Close()
}
...
func (e *externalInterface) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")
...
func (e *externalInterface) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalInterface) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalInterface) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
