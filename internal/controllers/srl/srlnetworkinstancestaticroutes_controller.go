...
const (
	...
	errUpdateNetworkinstanceStaticroutes           = "cannot update NetworkinstanceStaticroutes"
...
func SetupNetworkinstanceStaticroutes(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupNetworkinstanceStaticroutes(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.NetworkinstanceStaticroutesGroupKind)
	...
	r := managed.NewReconciler(mgr,
		...
		managed.WithValidator(&validatorNetworkinstanceStaticroutes{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("NetworkinstanceStaticroutes-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	NetworkinstanceStaticroutesHandler := &EnqueueRequestForAllNetworkinstanceStaticroutes{
	...
	return srlv1alpha1.NetworkinstanceStaticroutesGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlNetworkinstanceStaticroutes{}).
		Owns(&srlv1alpha1.SrlNetworkinstanceStaticroutes{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlNetworkinstanceStaticroutes{}}, NetworkinstanceStaticroutesHandler).
		Watches(&source.Channel{Source: events}, NetworkinstanceStaticroutesHandler).
		Complete(r)
}
...
func (r *NetworkinstanceStaticroutes) GetRootPath(mg resource.Managed) []*gnmi.Path {
...
func (r *NetworkinstanceStaticroutes) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
func (c *connectorNetworkinstanceStaticroutes) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalNetworkinstanceStaticroutes{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}
...
func (e *externalNetworkinstanceStaticroutes) Close() {
	e.client.Close()
}
...
func (e *externalNetworkinstanceStaticroutes) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")
...
func (e *externalNetworkinstanceStaticroutes) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
