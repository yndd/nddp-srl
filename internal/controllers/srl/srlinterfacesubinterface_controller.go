...
const (
	...
	errUpdateInterfaceSubinterface           = "cannot update InterfaceSubinterface"
...
func SetupInterfaceSubinterface(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupInterfaceSubinterface(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.InterfaceSubinterfaceGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangInterfaceSubinterface(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.InterfaceSubinterfaceGroupVersionKind),
		...
		managed.WithValidator(&validatorInterfaceSubinterface{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("InterfaceSubinterface-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	InterfaceSubinterfaceHandler := &EnqueueRequestForAllInterfaceSubinterface{
	...
	return srlv1alpha1.InterfaceSubinterfaceGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlInterfaceSubinterface{}).
		Owns(&srlv1alpha1.SrlInterfaceSubinterface{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlInterfaceSubinterface{}}, InterfaceSubinterfaceHandler).
		Watches(&source.Channel{Source: events}, InterfaceSubinterfaceHandler).
		Complete(r)
}
...
func initYangInterfaceSubinterface(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &InterfaceSubinterface{&yresource.Resource{
...
func (r *InterfaceSubinterface) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlInterfaceSubinterface)
	if !ok {
...
func (r *InterfaceSubinterface) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
func (v *validatorInterfaceSubinterface) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlInterfaceSubinterface)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedInterfaceSubinterface)
	}
	d, err := json.Marshal(&cr.Spec.InterfaceSubinterface)
	if err != nil {
...
func (c *connectorInterfaceSubinterface) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalInterfaceSubinterface{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}
...
func (e *externalInterfaceSubinterface) Close() {
	e.client.Close()
}
...
func (e *externalInterfaceSubinterface) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")
...
func (e *externalInterfaceSubinterface) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalInterfaceSubinterface) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalInterfaceSubinterface) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
