...
func SetupTunnelinterface(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	...
	TunnelinterfaceHandler := &EnqueueRequestForAllTunnelinterface{
	...
	return srlv1alpha1.TunnelinterfaceGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlTunnelinterface{}}, TunnelinterfaceHandler).
		Watches(&source.Channel{Source: events}, TunnelinterfaceHandler).
		Complete(r)
}
...
func (c *connectorTunnelinterface) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlTunnelinterface)
	if !ok {
		return nil, errors.New(errUnexpectedTunnelinterface)
	}
...
func (e *externalTunnelinterface) Close() {
	e.client.Close()
}
...
func (e *externalTunnelinterface) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")
