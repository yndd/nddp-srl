...
const (
	// Errors
	errUnexpectedNetworkinstanceNexthopgroups       = "the managed resource is not a NetworkinstanceNexthopgroups resource"
	errKubeUpdateFailedNetworkinstanceNexthopgroups = "cannot update NetworkinstanceNexthopgroups"
	errReadNetworkinstanceNexthopgroups             = "cannot read NetworkinstanceNexthopgroups"
	errCreateNetworkinstanceNexthopgroups           = "cannot create NetworkinstanceNexthopgroups"
	errUpdateNetworkinstanceNexthopgroups           = "cannot update NetworkinstanceNexthopgroups"
...
func SetupNetworkinstanceNexthopgroups(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupNetworkinstanceNexthopgroups(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.NetworkinstanceNexthopgroupsGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangNetworkinstanceNexthopgroups(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.NetworkinstanceNexthopgroupsGroupVersionKind),
		managed.WithPollInterval(nddcopts.Poll),
		managed.WithExternalConnecter(&connectorNetworkinstanceNexthopgroups{
			log:          nddcopts.Logger,
			kube:         mgr.GetClient(),
		...
		managed.WithValidator(&validatorNetworkinstanceNexthopgroups{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("NetworkinstanceNexthopgroups-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	NetworkinstanceNexthopgroupsHandler := &EnqueueRequestForAllNetworkinstanceNexthopgroups{
	...
	return srlv1alpha1.NetworkinstanceNexthopgroupsGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlNetworkinstanceNexthopgroups{}).
		Owns(&srlv1alpha1.SrlNetworkinstanceNexthopgroups{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlNetworkinstanceNexthopgroups{}}, NetworkinstanceNexthopgroupsHandler).
		Watches(&source.Channel{Source: events}, NetworkinstanceNexthopgroupsHandler).
		Complete(r)
}
...
func initYangNetworkinstanceNexthopgroups(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &NetworkinstanceNexthopgroups{&yresource.Resource{
...
func (r *NetworkinstanceNexthopgroups) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceNexthopgroups)
	if !ok {
...
func (r *NetworkinstanceNexthopgroups) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorNetworkinstanceNexthopgroups struct {
	log          logging.Logger
	deviceSchema *yentry.Entry
	y            yresource.Handler
}

func (v *validatorNetworkinstanceNexthopgroups) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceNexthopgroups)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedNetworkinstanceNexthopgroups)
	}
	d, err := json.Marshal(&cr.Spec.NetworkinstanceNexthopgroups)
	if err != nil {
...
func (v *validatorNetworkinstanceNexthopgroups) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
...
func (v *validatorNetworkinstanceNexthopgroups) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	//log := v.log.WithValues("resource", mg.GetName())
...
// is called.
type connectorNetworkinstanceNexthopgroups struct {
	log          logging.Logger
	kube         client.Client
...
func (c *connectorNetworkinstanceNexthopgroups) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	log := c.log.WithValues("resource", mg.GetName())
	//log.Debug("Connect")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceNexthopgroups)
	if !ok {
		return nil, errors.New(errUnexpectedNetworkinstanceNexthopgroups)
	}
	...
	return &externalNetworkinstanceNexthopgroups{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalNetworkinstanceNexthopgroups struct {
...
func (e *externalNetworkinstanceNexthopgroups) Close() {
	e.client.Close()
}

func (e *externalNetworkinstanceNexthopgroups) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Observing ...")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceNexthopgroups)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errUnexpectedNetworkinstanceNexthopgroups)
	}
...
func (e *externalNetworkinstanceNexthopgroups) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstanceNexthopgroups)
	if !ok {
		return errors.New(errUnexpectedNetworkinstanceNexthopgroups)
	}
...
func (e *externalNetworkinstanceNexthopgroups) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalNetworkinstanceNexthopgroups) Delete(ctx context.Context, mg resource.Managed) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Deleting ...")
...
func (e *externalNetworkinstanceNexthopgroups) GetTarget() []string {
	return e.targets
}

func (e *externalNetworkinstanceNexthopgroups) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalNetworkinstanceNexthopgroups) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
