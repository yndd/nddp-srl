...
func SetupSystemName(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	...
	SystemNameHandler := &EnqueueRequestForAllSystemName{
	...
	return srlv1alpha1.SystemNameGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlSystemName{}}, SystemNameHandler).
		Watches(&source.Channel{Source: events}, SystemNameHandler).
		Complete(r)
}
...
func initYangSystemName(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
...
func (v *validatorSystemName) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlSystemName)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedSystemName)
	}
...
func (c *connectorSystemName) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlSystemName)
	if !ok {
		return nil, errors.New(errUnexpectedSystemName)
	}
...
func (e *externalSystemName) Close() {
	e.client.Close()
}
...
func (e *externalSystemName) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlSystemName)
	if !ok {
		return errors.New(errUnexpectedSystemName)
	}
...
func (e *externalSystemName) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalSystemName) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
