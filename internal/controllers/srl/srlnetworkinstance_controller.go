...
const (
	...
	errUpdateNetworkinstance           = "cannot update Networkinstance"
...
func SetupNetworkinstance(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	//func SetupNetworkinstance(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) error {

	name := managed.ControllerName(srlv1alpha1.NetworkinstanceGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangNetworkinstance(
		nddcopts.DeviceSchema,
	)

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(srlv1alpha1.NetworkinstanceGroupVersionKind),
		...
		managed.WithValidator(&validatorNetworkinstance{
			log:          nddcopts.Logger,
			deviceSchema: nddcopts.DeviceSchema,
			y:            y},
		),
		managed.WithLogger(nddcopts.Logger.WithValues("Networkinstance-controller", name)),
		managed.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name))))

	NetworkinstanceHandler := &EnqueueRequestForAllNetworkinstance{
	...
	return srlv1alpha1.NetworkinstanceGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&srlv1alpha1.SrlNetworkinstance{}).
		Owns(&srlv1alpha1.SrlNetworkinstance{}).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlNetworkinstance{}}, NetworkinstanceHandler).
		Watches(&source.Channel{Source: events}, NetworkinstanceHandler).
		Complete(r)
}
...
func initYangNetworkinstance(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
	return &Networkinstance{&yresource.Resource{
...
func (r *Networkinstance) GetRootPath(mg resource.Managed) []*gnmi.Path {

	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstance)
	if !ok {
...
func (r *Networkinstance) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
func (v *validatorNetworkinstance) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlNetworkinstance)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedNetworkinstance)
	}
	d, err := json.Marshal(&cr.Spec.Networkinstance)
	if err != nil {
...
func (c *connectorNetworkinstance) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalNetworkinstance{client: cl, targets: tns, log: log, deviceSchema: c.deviceSchema, nddpSchema: c.nddpSchema, y: c.y}, nil
}
...
func (e *externalNetworkinstance) Close() {
	e.client.Close()
}
...
func (e *externalNetworkinstance) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")
...
func (e *externalNetworkinstance) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalNetworkinstance) GetConfig(ctx context.Context, mg resource.Managed) ([]byte, error) {
...
func (e *externalNetworkinstance) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
