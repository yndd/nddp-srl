...
func SetupSystemNtp(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {
	...
	SystemNtpHandler := &EnqueueRequestForAllSystemNtp{
	...
	return srlv1alpha1.SystemNtpGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		...
		*/
		Watches(&source.Kind{Type: &srlv1alpha1.SrlSystemNtp{}}, SystemNtpHandler).
		Watches(&source.Channel{Source: events}, SystemNtpHandler).
		Complete(r)
}
...
func initYangSystemNtp(deviceSchema *yentry.Entry, opts ...yresource.Option) yresource.Handler {
...
func (v *validatorSystemNtp) ValidateLeafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateLeafRefObservation, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlSystemNtp)
	if !ok {
		return managed.ValidateLeafRefObservation{}, errors.New(errUnexpectedSystemNtp)
	}
...
func (c *connectorSystemNtp) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	cr, ok := mg.(*srlv1alpha1.SrlSystemNtp)
	if !ok {
		return nil, errors.New(errUnexpectedSystemNtp)
	}
...
func (e *externalSystemNtp) Close() {
	e.client.Close()
}
...
func (e *externalSystemNtp) Create(ctx context.Context, mg resource.Managed, ignoreTransaction bool) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Creating ...")

	cr, ok := mg.(*srlv1alpha1.SrlSystemNtp)
	if !ok {
		return errors.New(errUnexpectedSystemNtp)
	}
...
func (e *externalSystemNtp) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) error {
	//log := e.log.WithValues("Resource", mg.GetName())
	//log.Debug("Updating ...")
...
func (e *externalSystemNtp) GetResourceName(ctx context.Context, mg resource.Managed, path *gnmi.Path) (string, error) {
